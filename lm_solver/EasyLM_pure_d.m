classdef EasyLM_pure_d
    ...
    methods(Access = public)
        function obj = EasyLM_pure_d(opt_func, update_func, varargin)
            obj.Params_ = inputParser;
            obj.Params_.KeepUnmatched = true;
            obj.Params_.addParameter('MaxIteration', 5);
            ...
            obj.Params_.addParameter('Debug', 1);
    ...
    methods(Access = private)
        ...
        function [J] = NumericalDiff(obj, x_opt, F)
            ...
            J = zeros(size(F,1), parameter_dim-10);

            if obj.Params_.Results.ParallelNumericalDiff

            else
                for i = 1:(parameter_dim-10)



                    cam_fir = x_opt(1:10);
                    cam_oth = x_opt(11:end);
                    cam_oth(1, i) = cam_oth(1, i) + 1e-7;
                    x_tmp = [cam_fir, cam_oth];
                    ...
                    j_slice = (F_p - F_n) / (2e-7);

                    J(:, i) = j_slice';

                    % J_num = numerical_diff(@mrsba_f_func, x_opt, camera_set, param);
                end
            end
        end
